...
const pickColumns = (items) => {
  const preferred = ["id", "title", "name", "duration", "intensity", "date", "type"];
...
export default function Workouts() {
  ...
  const fetchWorkouts = () => {
    ...
    fetch(apiUrl, { cache: "no-store" })
      ...
      .then((data) => {
        ...
        setWorkouts(normalizeData(data));
        setError(null);
      })
      .catch((fetchError) => {
  ...
  const columns = useMemo(() => pickColumns(filteredWorkouts), [filteredWorkouts]);
    ...
    <div className="card shadow-sm border-0">
      <div className="card-header d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between gap-3">
        <div>
          ...
          <p className="text-muted mb-0">Workout entries fetched from the backend REST API and displayed in a consistent table layout.</p>
        </div>
        <div className="btn-toolbar">
          <button type="button" className="btn btn-outline-primary me-2" onClick={fetchWorkouts}>
            Refresh
          </button>
          <button type="button" className="btn btn-secondary" onClick={() => setModalData({ section: "Workouts", data: filteredWorkouts })}>
            View raw JSON
          </button>
        </div>
      </div>
      <div className="card-body">
        ...
        <form className="row g-2 align-items-center mb-3" onSubmit={(evt) => evt.preventDefault()}>
          <div className="col-sm-8">
            <label htmlFor="workoutsSearch" className="visually-hidden">
              Search workouts
            </label>
            <input
              id="workoutsSearch"
          ...
          <div className="col-auto">
            <span className="text-muted">{filteredWorkouts.length} results</span>
          </div>
        </form>
            ...
            <table className="table table-hover table-bordered align-middle mb-0">
              ...
              <tbody>
                {filteredWorkouts.map((item, index) => (
                  <tr key={item.id || index} className="cursor-pointer" onClick={() => setModalData({ section: "Workouts", data: item })}>
                    {columns.map((column) => (
