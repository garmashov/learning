...
const pickColumns = (items) => {
  const preferred = ["id", "rank", "name", "username", "score", "points", "team"];
...
export default function Leaderboard() {
  ...
  const columns = useMemo(() => pickColumns(filteredItems), [filteredItems]);
    ...
    <div className="card shadow-sm border-0">
      <div className="card-header d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between gap-3">
        <div>
          ...
          <p className="text-muted mb-0">Top performers pulled from your REST API and rendered in a Bootstrap table.</p>
        </div>
        <div className="btn-toolbar">
          <button type="button" className="btn btn-outline-primary me-2" onClick={fetchLeaderboard}>
            Refresh
          </button>
          <button type="button" className="btn btn-secondary" onClick={() => setModalData({ section: "Leaderboard", data: filteredItems })}>
            View raw JSON
          </button>
        </div>
      </div>
      <div className="card-body">
        ...
        <form className="row g-2 align-items-center mb-3" onSubmit={(evt) => evt.preventDefault()}>
          <div className="col-sm-8">
            <label htmlFor="leaderboardSearch" className="visually-hidden">
              Search leaderboard
            </label>
            <input
          ...
          <div className="col-auto">
            <span className="text-muted">{filteredItems.length} entries</span>
          </div>
        </form>
        ...
        {isLoading ? (
          ...
          <div className="alert alert-warning">No leaderboard data available.</div>
        ) : (
            ...
            <table className="table table-hover table-bordered align-middle mb-0">
              ...
              <tbody>
                {filteredItems.map((item, index) => (
                  <tr key={item.id || index} className="cursor-pointer" onClick={() => setModalData({ section: "Leaderboard", data: item })}>
                    {columns.map((column) => (
